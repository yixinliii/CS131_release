...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    matches = []
    for i in range(M):
        sortedDI = np.sort(dists[i]) # Get the distances between desc1[i] and descriptors in desc[2]
        if sortedDI[0] / sortedDI[1] < threshold:
            # append the index of the vector in desc1 and the index of the vector in desc2
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    ### YOUR CODE HERE
        ...
        # 3. Compute inliers via Euclidean distance
        transformed2 = matched2 @ H_candidate
        distances = np.linalg.norm(transformed2[:, :2] - matched1[:, :2], axis=1)
    ...
    # 5. Re-compute least-squares estimate on all of the inliers
    inlier_p1 = pad(keypoints1[orig_matches[max_inliers, 0]])
    inlier_p2 = pad(keypoints2[orig_matches[max_inliers, 1]])
    H = fit_affine_matrix(inlier_p1, inlier_p2, to_pad=False)
    ### END YOUR CODE
...
def linear_blend(img1_warped, img2_warped):
    ...
    if right_margin > left_margin:
        ...
        linear_decrease = np.linspace(1, 0, blend_width)
        w1[left_margin:right_margin] = linear_decrease
        w1[right_margin:] = 0

    w2 = np.zeros(out_W)
    if right_margin > left_margin:
        blend_width = right_margin - left_margin
        linear_increase = np.linspace(0, 1, blend_width)
        w2[left_margin:right_margin] = linear_increase
        w2[right_margin:] = 1
    
    W1 = np.tile(w1, (out_H, 1))
    W2 = np.tile(w2, (out_H, 1))

    merged = W1 * img1_warped + W2 * img2_warped
    ### END YOUR CODE
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    affine_pairs = []
    for i in range(len(imgs) - 1):
        H_pair, _ = ransac(keypoints[i], keypoints[i + 1], matches[i])
        affine_pairs.append(H_pair)
    
    # Compute cumulative affine transformations
    cumulative_affines = [np.eye(3)]
    for H_pair in affine_pairs:
        cumulative_affines.append(H_pair @ cumulative_affines[-1])
    
    canvas = np.zeros((0, 0))
    global_offset = np.zeros(2)
    for im in imgs:
        current_canvas, current_offset = get_output_space(im, imgs, cumulative_affines)
        if current_canvas.shape[0] > canvas.shape[0]:
            canvas, global_offset = current_canvas, current_offset
    
    warped_imgs = []
    for idx, im in enumerate(imgs):
        warped = warp_image(im, cumulative_affines[idx], canvas, global_offset)
        # Replace fill value (-1) with 0 and build a valid pixel mask if needed.
        valid = (warped != -1)
        warped[~valid] = 0
        warped_imgs.append(warped)
    
    # Blend the warped images using linear blending.
    panorama = warped_imgs[0]
    for next_img in warped_imgs[1:]:
        panorama = linear_blend(panorama, next_img)
    ### END YOUR CODE
